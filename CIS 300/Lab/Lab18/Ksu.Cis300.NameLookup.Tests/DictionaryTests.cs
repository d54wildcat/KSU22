...
namespace Ksu.Cis300.NameLookup.Tests
{
    ...
    public class DictionaryTests
    {
        ...
        public void TestBAddOneRemoveIt()
        {
            ...
            Assert.Multiple(() =>
            {
                Assert.That(removeResult, Is.True, "The value returned by Remove is incorrect.");
                Assert.That(lookupResult, Is.False, "The key is still in the Dictionary.");
            });
        }
        ...
        public void TestDRemoveLeafThenItsParent()
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(lookup7Result, Is.EqualTo("Seven"));
                Assert.That(lookup10Result, Is.EqualTo("Ten"));
                Assert.That(lookup13Result, Is.Null, "When looking up 13, the out parameter is incorrect.");
                Assert.That(lookup15Result, Is.Null, "When looking up 15, the out parameter is incorrect.");
                Assert.That(lookup20Result, Is.EqualTo("Twenty"));
            });
        }
        ...
        public void TestDRemoveOtherLeafThenItsParent()
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(lookup15Result, Is.Null, "When looking up 15, the out parameter is incorrect.");
                Assert.That(lookup20Result, Is.Null, "When looking up 20, the out parameter is incorrect.");
            });
        }
        ...
        /// <summary>
        /// Builds a tree with 7 nodes, removes 3 of them, then looks up all of them.
        /// </summary>
        [Test, Timeout(1000)]
        public void TestFRemoveThree()
        {
            Dictionary<int, string> d = LoadDictionary();
            bool remove10Result = d.Remove(10);
            bool remove13Result = d.Remove(13);
            bool remove7Result = d.Remove(7);
            d.TryGetValue(3, out string lookup3Result);
            d.TryGetValue(5, out string lookup5Result);
            d.TryGetValue(7, out string lookup7Result);
            d.TryGetValue(10, out string lookup10Result);
            ...
            Assert.Multiple(() =>
            {
                Assert.That(remove10Result, Is.True, "Remove(10) returned false.");
                Assert.That(remove13Result, Is.True, "Remove(13) returned false.");
                Assert.That(remove7Result, Is.True, "Remove(7) returned false.");
                Assert.That(lookup3Result, Is.EqualTo("Three"));
                Assert.That(lookup5Result, Is.EqualTo("Five"));
                Assert.That(lookup7Result, Is.Null, "7 was not removed.");
                Assert.That(lookup10Result, Is.Null, "10 was not removed.");
                Assert.That(lookup13Result, Is.Null, "13 was not removed.");
                Assert.That(lookup15Result, Is.EqualTo("Fifteen"));
                Assert.That(lookup20Result, Is.EqualTo("Twenty"));
            });
        }
